...
async function getChatGPTPricingWithContext(
  currentPrice: number,
...
USE THESE PRICING GUIDELINES (THESE ARE THE GUIDELINES THAT I WILL REFER TO):
...
export class AIPricingService {
  async analyzePricing(
    ...
    try {
      const chatGPTResult = await getChatGPTPricingWithContext(
        originalPrice,
        input,
      );
      ...
      if (chatGPTResult.success && chatGPTResult.suggestedPrice) {
        ...
        const reasoning =
          Array.isArray(chatGPTResult.reasoning) &&
          chatGPTResult.reasoning.length > 1
