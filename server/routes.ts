...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/misbehavior-reports", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // Send misbehavior report - use working email since lendibl.com domain isn't configured for email
      const disputesEmail = 'arnav.nallani@gmail.com'; // Forward disputes to working email
      const emailSuccess = await sendEmail({
        to: disputesEmail,
        subject: `[DISPUTES] Misbehavior Report #${report.reportId} - ${report.incidentType}`,
        ...
        html: `
          <div style="background-color: #dc2626; color: white; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <p style="color: #fecaca; margin: 5px 0 0 0;">This email was sent to disputes@lendibl.com but forwarded here since the domain isn't configured for email</p>
          </div>
          ...
          <div style="margin-top: 20px; padding: 10px; background-color: #f3f4f6; border-radius: 6px;">
            <p style="margin: 0; color: #666; font-size: 14px;">
              <strong>Note:</strong> This report was automatically generated by the lendibl platform. 
              To properly receive these at disputes@lendibl.com, configure MX records for the lendibl.com domain.
            </p>
          </div>
        `,
        text: `
...
IMPORTANT: This was sent to disputes@lendibl.com but forwarded here since the domain isn't configured for email.
...
NOTE: To properly receive these at disputes@lendibl.com, configure MX records for the lendibl.com domain.
...
This report was automatically generated by the lendibl platform dispute system.
      ...
      if (emailSuccess) {
        console.log(`✅ Misbehavior report sent successfully to disputes team (${disputesEmail})`);
      } else {
        console.log(`❌ Failed to send misbehavior report to disputes team`);
      }
