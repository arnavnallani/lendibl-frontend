...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Real SMS verification (sends actual SMS codes)
  app.post("/api/auth/send-sms-verification", async (req, res) => {
    try {
      ...
      const result = await phoneVerificationService.sendSMSVerificationCode(phoneNumber);
      
      console.log(`✅ SMS send result: ${result.message}`);
      res.json(result);
    } catch (error) {
  ...
  app.post("/api/auth/verify-sms-code", async (req, res) => {
    try {
      ...
      console.log(`📱 Verifying SMS code for transaction: ${transactionId}`);
      
      const result = await phoneVerificationService.verifySMSCode(transactionId, verificationCode);
      
      console.log(`✅ SMS verification result: ${result.message}`);
      res.json(result);
    } catch (error) {
  ...
  app.post("/api/auth/send-email-verification", async (req, res) => {
    try {
      ...
      const result = await emailVerificationService.sendEmailVerification(email, firstName);
      
      console.log(`✅ Email send result: ${result.message}`);
      res.json(result);
    } catch (error) {
  ...
  app.post("/api/auth/verify-email-token", async (req, res) => {
    try {
      ...
      console.log(`✅ Email token verification result: ${result.message}`);
      res.json(result);
    } catch (error) {
