...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/misbehavior-reports", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // Send email to customerservice@lendibl.com (and copy to user for testing)
      const emailSuccess = await sendEmail({
        to: req.user?.email || 'customerservice@lendibl.com',
...
This report was automatically generated by the lendibl platform dispute system.
      ...
      if (emailSuccess) {
        console.log(`✅ Report email sent successfully to ${req.user?.email || 'customerservice@lendibl.com'}`);
      } else {
        console.log(`❌ Failed to send report email to ${req.user?.email || 'customerservice@lendibl.com'}`);
      }
      
      // Also send to customerservice@lendibl.com if we sent to user email
      if (req.user?.email && req.user.email !== 'customerservice@lendibl.com') {
        const disputesEmailSuccess = await sendEmail({
          to: 'customerservice@lendibl.com',
          subject: `Misbehavior Report #${report.reportId} - ${report.incidentType}`,
          type: 'misbehavior-report',
          html: `
            <h2>🚨 Misbehavior Report Submitted</h2>
...
This report was automatically generated by the lendibl platform dispute system.
        ...
        if (disputesEmailSuccess) {
          console.log('✅ Report also sent to customerservice@lendibl.com');
        } else {
          console.log('❌ Failed to send copy to customerservice@lendibl.com');
        }
      }
