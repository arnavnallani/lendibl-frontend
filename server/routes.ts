...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      // Track user in first 100 (excluding Arnav Nallani)
      try {
        const tracked = await storage.trackFirst100User(user.id, user.username, user.email);
        if (tracked) {
          console.log(`🎉 User ${user.username} is #${tracked.registrationOrder} in the first 100 users!`);
        }
      } catch (error) {
        console.log('Error tracking first 100 user:', error);
      }
  ...
  app.get("/api/ai-search", async (req, res) => {
    ...
    try {
      ...
      console.log(`🔍 Passing ${allItems.length} items to AI search:`, allItems.slice(0,3).map(i => i.title));
  ...
  app.put("/api/bookings/:id", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (updatedBooking && status === 'approved') {
        ...
        if (!canApprove) {
          return res.status(400).json({ 
            message: "Please set up a payment method first so you can be able to receive money after the transaction.",
      ...
      const emailSuccess = await sendEmail({
        ...
        html: `
          <div style="background-color: #dc2626; color: white; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            <h2 style="color: white; margin: 0;">🚨 LENDIBL MISBEHAVIOR REPORT</h2>
            <p style="color: #fecaca; margin: 5px 0 0 0;">Dispute report submitted through lendibl platform</p>
          </div>
          ...
          <div style="margin-top: 20px; padding: 10px; background-color: #f3f4f6; border-radius: 6px;">
            <p style="margin: 0; color: #666; font-size: 14px;">
              <strong>Note:</strong> This report was automatically generated by the lendibl platform dispute system.
            </p>
          </div>
        `,
        text: `
🚨 LENDIBL MISBEHAVIOR REPORT #${report.reportId}
...
This report was automatically generated by the lendibl platform dispute system.
      ...
      if (emailSuccess) {
        console.log(`✅ Misbehavior report sent successfully to disputes@lendibl.com`);
      } else {
        console.log(`❌ Failed to send misbehavior report to disputes@lendibl.com`);
      }
  ...
  // First 100 users tracking endpoint
  app.get("/api/admin/first-100-users", authenticateToken, async (req: AuthRequest, res) => {
    try {
      // Only allow Arnav Nallani to view this data
      const user = await storage.getUser(req.user!.id);
      if (!user || (!user.email.toLowerCase().includes('arnav.nallani') && !user.username.toLowerCase().includes('arnav'))) {
        return res.status(403).json({ message: "Access denied" });
      }

      const first100 = await storage.getFirst100Users();
      const count = await storage.getFirst100UserCount();

      res.json({
        count,
        users: first100.map(u => ({
          registrationOrder: u.registrationOrder,
          username: u.username,
          email: u.email,
      ...
      console.error("First 100 users error:", error);
      res.status(500).json({ message: "Failed to fetch first 100 users" });
    }
  });
